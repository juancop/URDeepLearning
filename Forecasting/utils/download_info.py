...
# Source: https://github.com/greatwhiz/tft_tf2/blob/master/script_download_data.py
# Utilizamos los data downloaders de los ejemplos para el TFT. 

# Modificado para la descarga específica de información.
...
def download_volatility(data_folder, file_name = 'volatility.csv'):
...
def download_electricity(data_folder, file_name = 'electricity.csv'):
...
def download_traffic(data_folder, file_name = 'traffic.csv'):
  ...
  output_file = os.path.join(data_folder, file_name)
  flat_df.to_csv(output_file)
  print('Done.')


def download_favorita(data_folder, file_name = 'favorita.csv'):
  ...
  outputs_file = os.path.join('train_data')
    
  
  shutil.unpack_archive(zip_file, os.path.join(data_folder, outputs_file))
  ...
  for file in glob.glob(os.path.join(data_folder, '*.7z')):
    ...
    print(csv_file)
    print(file)
    print(os.path.join(data_folder, csv_file))
    shutil.unpack_archive(file, os.path.join(data_folder, csv_file))
  ...
  #print(os.listdir('data/data/train.csv.7z'))
  temporal = pd.read_csv(os.path.join(data_folder,outputs_file, 'train.csv.7z'), index_col=0)

  store_info = pd.read_csv(os.path.join(data_folder, outputs_file, 'stores.csv.7Z'), index_col=0)
  oil = pd.read_csv(
